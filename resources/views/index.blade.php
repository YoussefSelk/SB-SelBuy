                ...
                <div class="flex items-center justify-center py-20">
                    <div class="bg-white rounded-xl shadow-lg p-8">
                        <div class="flex items-center mb-4">
                            <div class="mr-4">
                                <img src="https://via.placeholder.com/150x150.png?text=No+Image+Available" alt="No Image" class="w-16 h-16 object-cover rounded-full">
                            </div>
                            <div class="flex flex-col">
                                <div class="text-gray-800 font-medium">No Announcements</div>
                                <div class="text-gray-600 mt-2">Please check back later.</div>
                            </div>
                        </div>
                        <div class="p-4">
                            <p class="text-gray-600">There are currently no featured products. Please check back later for updates on our featured products.</p>
                        </div>
                    </div>
                </div>
            @else
                            ...
                            <div class="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-2xl transition-shadow duration-300">
                                <div class="flex items-center p-4 border-b border-gray-100">
                                    <div class="mr-4">
                                        @if ($product->user->avatar)
                                            <img src="{{ asset('storage/profile_pictures/' . $product->user->avatar) }}" alt="Profile Picture" class="w-12 h-12 rounded-full" alt="user photo">
                                        @else
                                            <img src="https://ui-avatars.com/api/?name={{ $product->user->name }}" alt="user photo" class="w-12 h-12 rounded-full">
                                        @endif
                                    </div>
                                    <div class="flex flex-col">
                                ...
                                @if ($product->images->count() > 0)
                                    @foreach ($product->images->take(1) as $image)
                                        <div class="p-4">
                                            <img src="{{ asset('images/' . $image->image_path) }}" alt="" class="w-full h-56 object-cover rounded-lg">
                                        </div>
                                    @endforeach
                                @else
                                    <div class="p-4">
                                        <img src="https://via.placeholder.com/300x200.png?text=No+Image+Available" alt="No Image Available" class="w-full h-56 object-cover rounded-lg">
                                    </div>
                                @endif
                                    ...
                                    <div class="">
                                        ...
                                        @if (auth()->check())
                                            @if (auth()->user()->hasFavorite($product->id))
                                                <button id="removeFromFavoritesBtn-{{ $product->id }}" class="remove-from-favorites-btn" data-favorite-id="{{ $product->id }}">Remove from Favorites</button>
                                            @else
                                                <button id="addToFavoritesBtn-{{ $product->id }}" class="add-to-favorites-btn" data-announcement-id="{{ $product->id }}">Add to Favorites</button>
                                            @endif
                                        @else
            ...
            @if ($carAnnouncements->isEmpty())
                <div class="flex items-center justify-center py-20">
                    <div class="bg-white rounded-xl shadow-lg p-8">
                        ...
                        <div class="p-4">
                            <p class="text-gray-600">There are currently no car announcements. Please check back later for updates on our car announcements.</p>
                        </div>
                    </div>
                </div>
            @else
            ...
            @if ($techAnnouncements->isEmpty())
                <div class="flex items-center justify-center py-20">
                    <div class="bg-white rounded-xl shadow-lg p-8">
                        ...
                        <div class="p-4">
                            <p class="text-gray-600">There are currently no tech announcements. Please check back later for updates on our tech announcements.</p>
                        </div>
                    </div>
                </div>
            @else
